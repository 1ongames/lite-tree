...
export default defineEventHandler(async (event) => {
  ...
  const user = db.prepare('SELECT name, email, password FROM users WHERE email = ? LIMIT 1').get(email) as { name: string; email: string; password: string } | undefined
  ...
  setCookie(event, 'session_token', jwt, { 
    httpOnly: true, 
    sameSite: 'lax', 
    path: '/', 
    secure: isProd, 
